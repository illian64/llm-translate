...
class FileProcessorHtml:
    ...
    def process_html(self, req: dto.ProcessingFileDirReq, soup: BeautifulSoup, body_tag, parallel: bool,
        ...
        children_list: list[PageElement] = []

        # generate list for separate step, because in some cases after insert text it break children iteration
        for child in children:
            if (child and child.text and child.parent and child.parent.get(self.attribute_translate) is None
                    and (child.parent.get(self.attribute_source) is None or not parallel)):
        ...
        for child in children_list:
            ...
            if child.parent.get(self.attribute_translate) is None and child.parent.get(self.attribute_source) is None:
                ...
                if parallel:
                    if original_text not in uniq_text_set:
                        uniq_text_set.add(original_text)
                        translate_params.append(translate_req)
                else:
                    translate_txt = self.core.translate(translate_req).result
                    if translate_txt == original_text:
                        continue
