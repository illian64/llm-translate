...
* **prompt** - инструкция для модели для перевода текста. В общем виде, инструкция определяет, что модель является
  ...
  Может быть в виде строки с названием файла (txt или json), файл должен находиться в директории проекта `external_prompt`. Например, `my_prompt.json`.
...
* **max_tokens_multiplier**: множитель максимального числа токенов в ответе, целочисленное.
  По умолчанию - 3, это значит, что будет взята длина текста, умножена на значение параметра и результат будет использован, как максимальное чилсо токено в генерации.
Следует учесть, что число токенов не равно числу символов в тексте, токенов обычно меньше.
Число токенов необходимо ограничивать, так как в некоторых случаях модель может "сломаться" и генерировать токены без остановки.
Редко бывает при переводе фраз с несколькими одинаковыми буквами в слове (вида "хааааааа!", "бзззззззз").
